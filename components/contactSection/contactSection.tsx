...
export function ContactSection({}) {
  ...
  return (
    <section className="section section__5 third darkGradient ">
      <Bulge type="Light" />
      {/* logos at the top of contact section */}
      <div className="pt-8 flex justify-center items-center space-x-8">
        <Magentic
          href={links.home}
          strength={50}
          className="nav__item anime text-xl font-bold text-colorLight before:bg-colorLight"
          onMouseEnter={() => {
        ...
        >
          <p className="mask logo__anim flex items-center justify-center font-semibold">
            ...
            >
              <path
                className="contact-logo__rotate"
      ...
      <header className="nav__container anime px-paddingX absolute z-[5000] left-0 top-0 w-full">
        <nav className="nav__bar">
          <div className="max-w-maxWidth">
            ...
            <Magentic
              strength={50}
              className="anime mask nav__item h-8 w-8 cursor-pointer items-center text-colorLight before:bg-colorLight"
              onClick={() => {
                dispatch(toggleMenu({ isMenuOpen: true, color: "Light" }));
              }}
            >
              <div className="flex h-[0.9rem] w-full flex-col justify-between">
                <div className="h-[0.15rem] w-full bg-colorLight"></div>
                <div className="h-[0.15rem] w-full bg-colorLight"></div>
              </div>
            </Magentic>
          </div>
      ...
      {/* contact details instead of the rotating "Contact" link */}
      <div className="footer__heading anime text-center py-12">
        <p className="text-colorLight text-lg md:text-xl">
          Get in touch:
        </p>
        <a
          href={links.email}
          className="mt-2 inline-block text-colorLight underline hover:opacity-80"
        >
